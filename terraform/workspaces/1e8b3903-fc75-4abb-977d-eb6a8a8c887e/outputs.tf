...
output "bucket_info" {
  ...
  value = {
    bucket_name         = module.main_bucket.bucket_name
    bucket_arn          = module.main_bucket.bucket_arn
    bucket_id           = module.main_bucket.bucket_id
    bucket_domain_name  = module.main_bucket.bucket_domain_name
    region             = module.main_bucket.region
  }
}
...
output "backup_bucket" {
  ...
  value = var.backup_enabled ? {
    enabled             = true
    backup_bucket_name  = module.backup_bucket[0].bucket_name
    backup_bucket_arn   = module.backup_bucket[0].bucket_arn
...
output "quick_start_guide" {
  ...
  value = {
    bucket_name = module.main_bucket.bucket_name
    steps = concat([
      "1. Your S3 bucket '${module.main_bucket.bucket_name}' is now ready",
      "2. Upload files: aws s3 cp ./file.txt s3://${module.main_bucket.bucket_name}/",
      "3. List contents: aws s3 ls s3://${module.main_bucket.bucket_name}/",
      "4. Download files: aws s3 cp s3://${module.main_bucket.bucket_name}/file.txt ./downloaded-file.txt",
      var.website_enabled ? "5. Website URL: http://${module.main_bucket.website_endpoint}" : "5. Website hosting disabled",
      var.backup_enabled ? "6. Backup bucket: ${module.backup_bucket[0].bucket_name}" : "6. No backup bucket configured",
      "7. Console access: https://s3.console.aws.amazon.com/s3/buckets/${module.main_bucket.bucket_name}"
    ], var.ec2_enabled ? [
      "",
...
output "resource_ids" {
  ...
  value = {
    main_bucket_name    = module.main_bucket.bucket_name
    main_bucket_arn     = module.main_bucket.bucket_arn
    backup_bucket_name  = var.backup_enabled ? module.backup_bucket[0].bucket_name : null
